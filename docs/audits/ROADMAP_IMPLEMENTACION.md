...
## FASE 1d — Reportes contables (continuación inmediata, RIESGO BAJO — solo lectura) ✅ COMPLETADA
...
## FASE 2 — Bancos y Conciliación (RIESGO MEDIO — nuevo modelo de datos) ✅ COMPLETADA
...
## FASE 3 — Inventario Avanzado (RIESGO ALTO — toca lógica de stock existente) ✅ COMPLETADA
...
## FASE 4 — Integraciones Reales (RIESGO MEDIO — requiere credenciales externas del usuario) ⚠️ PARCIALMENTE COMPLETADA — código listo, pendiente credenciales externas

✅ **Tarea 4.1** — Email real con Resend o SendGrid, reemplazando el `setTimeout` mock en `sendSupplierEmailAction` (suppliers.ts). Requiere que el usuario cree una cuenta y provea API key. *(Implementado con nodemailer SMTP genérico, solo falta configurar variables de entorno.)*
⚠️ **Tarea 4.2** — WhatsApp Business API real (o integración con Yappy que el usuario mencionó que tiene API propia) para envío de facturas/recordatorios de cobro. Disparo ya implementado (`src/lib/integrations/whatsapp.ts`) y gateado correctamente (no hace nada sin `whatsappPhone`/`whatsappToken` configurados) — falta que una empresa real registre credenciales y, para mensajes iniciados por el negocio, un template aprobado en Meta Business Manager (hoy usa `type: "text"`, solo válido dentro de la ventana de 24h). *(Código implementado en `src/lib/integrations/whatsapp.ts`, gateado correctamente, pendiente credenciales reales.)*
⚠️ **Tarea 4.3** — Webhooks reales: disparar POST HTTP a la URL configurada en Empresa cuando ocurren eventos (factura creada, pago recibido). Disparo ya implementado (`src/lib/integrations/webhooks.ts`) y gateado correctamente (no hace nada sin `webhookUrl` configurado) — falta que alguna empresa real configure uno para probarse en producción. *(Código implementado en `src/lib/integrations/webhooks.ts`, gateado correctamente, pendiente que una empresa configure URL.)*
⚠️ **Tarea 4.4** — PayPal con planes reales, reemplazando los IDs mock (`P-MOCK-BASIC-PLAN`) por planes creados de verdad en el dashboard de PayPal. *(Webhook funcional, usa IDs mock en desarrollo, pendiente planes reales en PayPal dashboard.)*
...
✅ Completado y verificado: Fases 0, 1, 1d (reportes contables), 2 (bancos y conciliación), 3 (inventario avanzado).
⚠️ Parcialmente completo: Fase 4 (integraciones — código listo, pendiente credenciales externas del usuario).
🔜 Siguiente: Fase 5 (DGI/PAC — diferida hasta confirmar contrato con efacturapty).
📋 Baja prioridad: Fase 6 (POS externos, biometría, 2FA, auditoría extendida).
