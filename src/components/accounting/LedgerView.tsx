    ...
    <ContentContainer>
      <div className="space-y-6">
        ...
        <Card className="bg-white/80 backdrop-blur-md border-slate-100 shadow-sm rounded-2xl">
          <CardContent className="p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex-1 space-y-1.5">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Seleccionar Cuenta Contable</label>
              <Select value={cuentaId} onValueChange={handleCuentaChange}>
                <SelectTrigger className="h-11 text-xs sm:text-sm bg-slate-50/50 border-slate-200 rounded-xl w-full md:max-w-md">
                  <SelectValue placeholder="Selecciona una cuenta..." />
                </SelectTrigger>
                <SelectContent className="rounded-xl max-h-80">
                  <SelectItem value="none" className="text-slate-400 cursor-pointer">Selecciona una cuenta...</SelectItem>
                  {cuentas.map((c) => (
            ...
            {selectedCuenta && (
              <div className="flex items-center gap-3 bg-slate-50/80 border border-slate-100 rounded-xl px-4 py-3">
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Detalles Cuenta</div>
                  <div className="text-xs font-semibold text-slate-700 mt-0.5">
                    Tipo: <span className="font-bold text-slate-900">{selectedCuenta.tipo}</span>
                  </div>
                  <div className="text-xs font-semibold text-slate-700 mt-0.5">
                    Naturaleza: <span className="font-bold text-slate-900">{selectedCuenta.naturaleza}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        {selectedCuentaId === 'none' || !selectedCuentaId ? (
          <Card className="bg-white/80 border border-slate-100 rounded-2xl p-12 text-center shadow-xs">
            <BookOpen className="mx-auto h-12 w-12 text-slate-300 mb-4" />
            <h3 className="text-base font-bold text-slate-700 mb-1">Libro Mayor</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card className="bg-white/50 backdrop-blur-md border-slate-100 shadow-sm rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-md">
                <CardContent className="p-6">
                  ...
                  <div className="mt-2 text-2xl font-bold text-slate-800 font-mono">{formatCurrency(totalDebe)}</div>
                </CardContent>
              </Card>
              <Card className="bg-white/50 backdrop-blur-md border-slate-100 shadow-sm rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-md">
                <CardContent className="p-6">
                  ...
                  <div className="mt-2 text-2xl font-bold text-slate-800 font-mono">{formatCurrency(totalHaber)}</div>
                </CardContent>
              </Card>
              <Card className="bg-slate-800 border-slate-900 shadow-sm rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-md">
                <CardContent className="p-6">
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-b border-slate-100 hover:bg-transparent">
                    ...
                    <TableHead className="text-xs font-bold text-slate-500 uppercase tracking-wider">Asiento</TableHead>
                    ...
                    <TableHead className="text-xs font-bold text-slate-500 uppercase tracking-wider">Detalle/Descripción</TableHead>
                    ...
                    <TableHead className="text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Saldo Acumulado</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {movements.length === 0 ? (
                    ...
                    movements.map((mov) => (
                      <TableRow key={mov.id} className="border-b border-slate-100/50 hover:bg-slate-50/10">
                        ...
                        <TableCell className="text-slate-750 text-xs font-medium max-w-xxs truncate">
                          {mov.concepto}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-slate-500 text-xs max-w-xxs truncate">
                          {mov.descripcion || '-'}
                        </TableCell>
