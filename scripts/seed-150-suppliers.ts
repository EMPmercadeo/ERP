...
const ESTADOS = ['activo', 'activo', 'activo', 'activo', 'activo', 'activo', 'activo', 'inactivo', 'archivado', 'bloqueado'];
...
async function main() {
    console.log('🚀 Iniciando generación de 150 proveedores con facturas y pagos...');

    const empresas = await prisma.empresa.findMany();
    if (empresas.length === 0) {
        console.error('❌ No se encontraron empresas registradas en la base de datos.');
        return;
    }

    // Tomamos todas las empresas o la primera empresa activa
    for (const empresa of empresas) {
        ...
        for (let i = 1; i <= 150; i++) {
            ...
            // Para aproximadamente el 45% de los proveedores activos, vamos a crear 1 a 4 facturas de compra
            if (proveedor.estado === 'activo' && Math.random() < 0.45) {
                ...
                for (let f = 1; f <= numFacturas; f++) {
                    ...
                    // Decidir estado del documento: 40% pagada, 35% pendiente, 15% vencida, 10% parcial
                    ...
                    // Si hubo pago (total o parcial), creamos el registro en PagoProveedor
                    if (montoPagado > 0) {
                ...
                // Actualizar el saldo cacheado en Proveedor
        ...
        console.log(`✅ Empresa ${empresa.razonSocial}: ${creadosCount} proveedores con ${comprasCount} facturas y ${pagosCount} pagos.`);
    }
}
